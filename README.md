...
이번 턴 신규 1,420 tokens (입력 1,100 + 출력 320)
캐시 재사용 7,000 tokens (입력의 86.42%) · 총 관측 8,420 tokens (작업 7,180 + 판별 1,240)
세션 17턴 · Luna 7 / Terra 4 / Sol 4 / Astra 2 · 관측 누적 142,380 tokens (17/17턴) · Astra/Ultra 기준 비용 절감 추정 51%
```

집계는 현재 Router 프로세스의 해당 thread 기준입니다. 완전한 usage 분해를 받으면 `이번 턴 신규`는 미캐시 입력과 출력의 합계로, `캐시 재사용`은 이전 입력 문맥을 다시 참조한 양으로 표시합니다. 캐시 입력도 요청에 포함되고 할인된 사용량일 수 있으므로 0으로 취급하지 않으며, `총 관측`과 `관측 누적`은 캐시를 포함한 본작업·판별 usage의 합계입니다. 캐시 분해를 받지 못하면 기존처럼 `이번 턴 관측`만 표시합니다. 괄호의 `17/17턴`은 작업과 판별 사용량을 모두 수신한 턴 수입니다. App Server 문서가 이 이벤트를 결제 원장이나 모든 하위 작업의 완전한 합계로 보장하지 않으므로 `실측 청구량`이라고 부르지 않습니다. 작업 또는 판별 사용량이 하나라도 없으면 임의의 0이나 평균값을 만들지 않고 `관측 사용량 미수신`으로 표시합니다.
...
| 새 푸터의 실제 GUI 렌더링 | ✅ `DEEP: Sol / High`와 캐시·미캐시·출력 분해 표시 확인 |
...
평가가 이미 실행 중이면 추가 실행은 즉시 종료되며, 진행 보고서는 원자적으로 교체되어 재시작이나 중복 실행 중에도 깨진 JSON을 남기지 않습니다.
